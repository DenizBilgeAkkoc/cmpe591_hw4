...
if __name__ == "__main__":
    ...
    for i in range(10000):
        ...
        for p in curve:
            ...
            states.append(env.high_level_state()) # which is [ee_pos, obj_pos, [obj_height]] does not contain x value
            #[[ey, ez], [oy, oz], h]
        states = np.stack(states)
        states_arr.append(states)
        print(f"Collected {i+1} trajectories.", end="\r")
    # Added this part to save the dataset in the end
    # Convert states to data format
    all_data = []
    for states in states_arr:
        t = np.linspace(0, 1, states.shape[0])
        ey, ez, oy, oz, h = states[:, 0], states[:, 1], states[:, 2], states[:, 3], states[:, 4]
        all_data.append(np.stack([t, ey, ez, oy, oz, h], axis=1))

    output_path = "dataset_5000.npz"
    np.savez(output_path, data=np.array(all_data))
